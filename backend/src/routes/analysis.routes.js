...
const { getAnalysisJobStatus, getJobHistory, getJobResult } = require('../controllers/results.controller');

router.post('/submit', requireAuth, uploadVideo.single('video'), submitVideo);
router.get('/jobs', requireAuth, getJobHistory);
router.get('/jobs/:id/status', requireAuth, getAnalysisJobStatus);
